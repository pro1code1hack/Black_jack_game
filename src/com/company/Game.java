...
public class Game {
    ...
    public Game(JFrame f) {//with this constructor, we initialize the instant fields accordingly. This constructor gets a JFrame as a parameter.
        ...
        frame = f;
        faceDown = true;
        dealerWon = true;           // casino always wins!!!!
        roundOver = false;
    }
    ...
    public void formGame() {//this method will help us create the background of our game.
        ...
        btnRanks = new JButton("CARD RANKS");
        btnRanks.setBounds(930, 50, 190, 50);
        btnRanks.setFont(new Font("Comic Sans MS", Font.BOLD, 16));
        ...
        btnExit.addActionListener(new ActionListener() { //we add a action listener to the exit button.
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(frame, "Ви вийшли з балансом:" +  MAIN.currentBalance + "."); //we print out the message by getting our current balance from the Tester class.
                System.exit(0); //we exit the program.
            }
        });
        btnRanks.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                Ranks rules = new Ranks();
            }
        });
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if ((getSumOfHand(dealerHand)<21) && getSumOfHand(playerHand)<21) {//if both hands didn't reach 21, we check which hand is better and print out the result.
                    if(getSumOfHand(playerHand) > getSumOfHand(dealerHand)) {
                        faceDown = false;
                        dealerWon = false;
                        JOptionPane.showMessageDialog(frame, Registration.nickname +" HAS WON BECAUSE OF A BETTER HAND!");
                       // rest();
                        roundOver = true;
                    }
                    else {
                        faceDown = false;
                        JOptionPane.showMessageDialog(frame, "CASINO HAS WON BECAUSE OF A BETTER HAND!");
                        //rest();
                        roundOver = true;
                    }
                }
    ...
    public void checkHand (ArrayList<Card> hand) {//this method literally checks the hand for a blackjack or bust.
        if (hand.equals(playerHand)) { //checks if the parameter is player's hand.
            if(getSumOfHand(hand) == 21){ //if it is 21, player has done blackjack and the game is over.
                ...
                JOptionPane.showMessageDialog(frame, Registration.nickname +" HAS DONE BLACKJACK!" + Registration.nickname +" HAS WON!"); //we print out the result ot JOptionPane.
                //rest();           // ТУТ МЫ БЛОКИРУЕМ ПОТОК
                roundOver = true;
            }
            else if (getSumOfHand(hand) > 21) { //if it is bigger than 21, then the player hand has busted, dealer has won.
                faceDown = false; JOptionPane.showMessageDialog(frame,  Registration.nickname +" HAS BUSTED! CASINO HAS WON!");
              //  rest();
                roundOver = true;
            }
        }
            ...
            if(getSumOfHand(hand) == 21) { //we basically look for the same things we looked for the player's hand.
                faceDown = false;
                JOptionPane.showMessageDialog(frame, "CASINO HAS DONE BLACKJACK! CASINO HAS WON!");
                //rest();
                roundOver = true;
            }
                ...
                faceDown = false;
                dealerWon = false;
                JOptionPane.showMessageDialog(frame, "CASINO HAS JUST BUSTED! " + Registration.nickname +" HAS WON!");
               // rest();
                roundOver = true;
            }
        }
    ...
    // Реализация многопоточности между событиями - лаба Каплунпо многопоточности
    public static void rest() {
        try {
            Thread.sleep(500);//equal to 1 second.
        }
